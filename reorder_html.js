const fs = require('fs');
const lines = fs.readFileSync('stitch-prototype.html', 'utf-8').split('\n');
...
const gastronomia = getBlock('id="banquetes"', '</section>');
...
const ubicacion = getBlock('id="ubicacion"', '</section>');
...
const gastronomiaBlock = adjustStart(gastronomia);
...
const ubicacionBlock = adjustStart(ubicacion);
...
if(startReplace === -1 || endReplace === -1) {
    console.log('Error finding boundaries in html');
    process.exit(1);
}
...
fs.writeFileSync('stitch-prototype.html', newLines.join('\n'));
console.log('Reordered html successfully');
